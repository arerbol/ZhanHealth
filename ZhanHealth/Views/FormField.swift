 ...
 */
@IBDesignable
open class FormTextField: UITextField { // swiftlint:disable:this type_body_length
...
//    }
  ...
  fileprivate final func init_SkyFloatingLabelTextField() {
    ...
    configuresForThisApp()
  }
  
  fileprivate func configuresForThisApp() {
    self.clearButtonMode = .unlessEditing
    self.autocapitalizationType = .none
    self.lineHeight = 1.heightProportion()
    self.selectedLineColor = AppColor.green
    self.selectedLineHeight = 2.heightProportion()
    self.selectedTitleColor = AppColor.blue
    self.tintColor = AppColor.blue
    self.placeholderFont = UIFont(name: Font.medium.rawValue, size: 11.widthProportion())
    self.titleFont = UIFont(name: Font.medium.rawValue, size: 11.widthProportion())!
    self.font = UIFont(name: Font.regular.rawValue, size: 15.widthProportion())
    self.addTarget(self, action: #selector(textFieldStartChange(_:)), for: .editingDidBegin)
  }
  
  @objc func textFieldStartChange(_ textfield: FormTextField) {
    textfield.errorMessage = ""
  }
  
  fileprivate func addEditingChangedObserver() {
    self.addTarget(self, action: #selector(FormTextField.editingChanged), for: .editingChanged)
  }
